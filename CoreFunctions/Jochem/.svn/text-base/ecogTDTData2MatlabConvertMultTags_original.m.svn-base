function ecogTDTData2MatlabConvertMultTags_original(rawDataPath,blockName,varargin)
%function
...
if nargin<3
    ...
    for k=1:size(paramsGlobal.tagNames,1)
        if ~isempty(paramsGlobal.nChannels(k)) & paramsGlobal.nChannels(k)>0
            sampFreq = ecogTDTData2Matlab_original(rawDataPath,blockName,1:paramsGlobal.nChannels(k),paramsGlobal.tagNames(k,:),rawDataPath,paramsGlobal.tagNames(k,:));
        end
    end
end
        ...
        for m=1:size(paramsGlobal.tagNames,1)
            ...
            if strcmp(tagNames{k},paramsGlobal.tagNames(m,:))
                ...
                if ~isempty(nChannels) & nChannels>0
                    sampFreq = ecogTDTData2Matlab_original(rawDataPath,blockName,nChannels,tagNames{k},rawDataPath,tagNames{k});
                end
            end
        end
        if ~tagExistsFlag
