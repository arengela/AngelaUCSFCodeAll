...
% Last Modified by GUIDE v2.5 08-Jun-2011 17:16:52
...
ymax=max(max(plotData));
ymin=min(min(plotData));

BAD_INTERVALS=handles.badIntervals;


%{
for i=1:size(BAD_INTERVALS,2)
    samp=nearly(BAD_INTERVALS(i,1:2),timebaseGuiUnits)
    plot(timebaseGuiUnits(samp(1):samp(2)),plotData(badch,samp(1):samp(2)),'m');
end
%}
...
BAD_INTERVALS=handles.badIntervals;
    for i=1:size(BAD_INTERVALS,1)
        if ~isfield(handles.ecog,'hfo')
            h=patch([BAD_INTERVALS(i,1),BAD_INTERVALS(i,1),BAD_INTERVALS(i,2),BAD_INTERVALS(i,2)],[ymin, ymax, ymax, ymin],'y');
            set(h,'FaceAlpha',.2)
            set(h,'EdgeColor','none')
        
        else
            c=find(channelsToShow==BAD_INTERVALS(i,3));
            if ~isempty(c)
                ymin2=min(plotData(c,:));
                ymax2=max(plotData(c,:));

                h2=patch([BAD_INTERVALS(i,1)-1,BAD_INTERVALS(i,1)-1,BAD_INTERVALS(i,2)+1,BAD_INTERVALS(i,2)+1],[ymin2, ymax2, ymax2, ymin2],'m');
                set(h2,'FaceAlpha',.5)
...
%{    
    for i=1:size(BAD_INTERVALS,1)
        for j=1:length(badch)
            ymax2=max(max(plotData(badch(j,:))));
            ymin2=min(min(plotData(badch(j,:))));
            h=patch([BAD_INTERVALS(i,1),BAD_INTERVALS(i,1),BAD_INTERVALS(i,2),BAD_INTERVALS(i,2)],[ymin2, ymax2, ymax2, ymin2],'c');
            set(h,'FaceAlpha',.5)
            set(h,'EdgeColor','none')
        end
    end
...
% --- Executes on button press in deleteLastInterval.
function deleteLastInterval_Callback(hObject, eventdata, handles)
% hObject    handle to deleteLastInterval (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
BAD_INTERVALS=handles.badIntervals;
%global BAD_INTERVALS
BAD_INTERVALS(end,:)=[];
guidata(hObject, handles);
handles.badIntervals=BAD_INTERVALS;
