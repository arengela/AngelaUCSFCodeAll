...
window_before_event=sampling_rate/2;
...
if number_of_analog==2
    window_after_event=window_before_event+2.5*sampling_rate;
    eventIndices=zeros(3,50);
else
    window_after_event=window_before_event;
    eventIndices=zeros(1,50);
end

window_around_event=window_before_event+window_after_event;
 number_of_sec2read = window_around_event/sampling_rate; %******Does this need to change?
...
if number_of_analog==2
    fh_event2=figure(4);
    set(fh_event2,'Name','Event 2:Average Event Time Frequency Plot','NumberTitle','off');clf;      
    z=axes('Position',[.1 .1 .85 .85],'visible','off');
...
%%
% coordinates for the electrode numbers for 1 event (and if segment before
% and after event are equal
...
fig_num_x = [.5:(window_before_event*2+1):to_plot_grid*(window_before_event*2+1)];
...
sigfig_dash_y = repmat([0 num_singleEvent_freqs2plot*to_plot_grid],1)';
fig_dash_x = repmat([round((window_before_event*2+1)/2):window_before_event*2+1:to_plot_grid*+window_before_event*2],2,1);

%if number_of_analog==2
    % coordinates for the electrode numbers
    fig_num_y2 = [1.1:num_avgEvent_freqs2plot:to_plot_grid*num_avgEvent_freqs2plot];
    fig_num_y2 = reshape(repmat(fig_num_y2,[to_plot_grid 1]),number_of_electrodes_total,1);
    fig_num_x2 = [.5:(window_around_event+1):to_plot_grid*(window_around_event+1)];
    fig_num_x2 = reshape(repmat(fig_num_x2,[1 to_plot_grid]),number_of_electrodes_total,1);
    fig_nums2 = {};
    for i = 1:number_of_electrodes_total
    ...
    fig_dash_x2= repmat([round((window_before_event+1)):window_around_event+1:to_plot_grid*+window_around_event],2,1);
%end
...
if calculated_baseline_flag==1
    ...
    averages=repmat(mean(averages,3),[1 1 window_around_event+1]);
    stdevs=repmat(mean(stdevs,3),[1 1 window_around_event+1]);
end
...
if DA.ConnectServer('Local')>0 %Checks to see if there is an OpenProject and Workbench load
    %     if DA.SetSysMode(2)==1%Starts OpenEx projefct running and acquiring data. 1 = Standby, 2 = Preview, !!MUST CHANGE TO 3 WHEN RECORDING
...
%                     else                    
%                        %Save only 1 value for baseline 
%                      [averages, stdevs, medians]=calculate_baseline_angela(time2collectBaseline, ...
%                         sproj_name,dproj_name, points_needed4baseline, BufferSize, DA, number_of_channels, ...
%                         num_freq_bands, sampling_rate,number_of_sec2read);
%                     end
                    pause(1)
                ...
                if event_flag ==1
                    ...
                    event=(ANNewData_finalMAT(desired_ANchan2,:)>threshold);
...
%                     hold on
%                     plot(matlab_loopCounter,detected_num_events,'go');
%                     grid on;
...
%                             indLastEvent, eventIndices, number_of_analog);
                            ...
                            [num_events,indLastEvent,eventIndices,newEventFlag] = ...
                            parseEvents(ANNewData_finalMAT, ANfinalPos, desired_ANchan,...
                            desired_ANchan2, threshold, threshold2, sampling_rate, num_events,...
                            indLastEvent, eventIndices, number_of_analog,window_before_event);
%??????????????           
%{             
                        ...
                        if  newEventFlag==1 && number_of_analog ==1
                            ...
                            %Extract data segment for 1 event
                            [eventRelatedAvg,last_used_ind,allStacked,new_plot_flag, plotted_num_events,eventIndices,lastSpec,~,~] =...
                                extract_event_windows(num_events, plotted_num_events, eventIndices, window_before_event,window_after_event,...
                                DataAfterCAR,finalPos(1),number_of_electrodes_total ,num_avgEvent_freqs2plot, avgEvent_freqs2plot,freq_band_singlestacked, ...
                                eventRelatedAvg,[], last_used_ind,allStacked,averages, stdevs,number_of_analog);
                            

%                             [eventRelatedAvg,num_samples,last_used_ind,allStacked,new_plot_flag,good_event_count,lastSpec] = ...
%                                 average_event_window_angela(num_events,eventIndices(1,1:num_events) , window_around_event,...
%                                 DataAfterCAR,finalPos(1),number_of_electrodes_total, num_avgEvent_freqs2plot, avgEvent_freqs2plot,...
...
%                             
                        ...
                        elseif newEventFlag==1 && number_of_analog ==2     
                            %extract data segment for 2 events
                            
                            [eventRelatedAvg,last_used_ind,allStacked,new_plot_flag, plotted_num_events,eventIndices,lastSpec,lastSpec_event2,eventRelatedAvg2] =...
                                extract_event_windows(num_events, plotted_num_events, eventIndices, window_before_event,window_after_event,...
                                DataAfterCAR,finalPos(1),number_of_electrodes_total ,num_avgEvent_freqs2plot, avgEvent_freqs2plot,freq_band_singlestacked, ...
                                eventRelatedAvg,eventRelatedAvg2, last_used_ind,allStacked,averages, stdevs,number_of_analog);
                            
                            
%                             [eventRelatedAvg,last_used_ind,allStacked,new_plot_flag,plotted_num_events,lastSpec,lastSpec_event2,eventRelatedAvg2,eventIndices] =...
%                                 average_event_window_2_events(num_events, eventIndices, window_before_event,window_after_event,...
%                                 DataAfterCAR,finalPos(1),number_of_electrodes_total ,num_avgEvent_freqs2plot, avgEvent_freqs2plot,...
%                                 eventRelatedAvg, last_used_ind,allStacked,averages, stdevs,eventRelatedAvg2,freq_band_singlestacked,plotted_num_events);
%                
    end
                        %If plots need to be updated
                        if  new_plot_flag==1 & newEventFlag==1
                            ...
                            set(handles.figure1,'CurrentAxes',fh5)
                            hold on
                            plot(matlab_loopCounter,plotted_num_events,'r*');
                            
                            %%
                            %Plot average Spectrogram(s) around event(s)
                            
                            if number_of_analog ==2
                                %If 2 event trial, plot ave spectrograms
                                %around event 1 and event 2 in two separate
                                %figures
                                %All values should be already zscored when
                                %they come out of calculate ave function
 %*************Need to double check, esp for 1 event                               
                                ZscoreEventRelatedAvg=eventRelatedAvg;
                                ...
                                %event 2
                                ZscoreEventRelatedAvg=eventRelatedAvg2;
                                to_plot=  reshape_3Ddata(ZscoreEventRelatedAvg, window_before_event*2,...
                                    num_avgEvent_freqs2plot, to_plot_grid);
                                to_plot = artifactrejection(to_plot, window_before_event*2,-1.5, 1.5, .5, .8, to_plot_grid);
                            ...
                            line(fig_dash_x2,sigfig_dash_y2,'LineStyle','--','linewidth',1','color',[0 0 0]);
                            set(fh4,'Xtick',[0.5: (window_around_event+1):to_plot_grid*(window_around_event+1)])
                            set(fh4,'YTick',[0.5: num_singleEvent_freqs2plot:to_plot_grid*num_singleEvent_freqs2plot])
                            set(fh4,'YTickLabel','')
                            set(fh4,'XTickLabel','')
                            ...
                            to_plot=allStacked(:,1:plotted_num_events,:);
                            
                            set(fh6,'DefaultAxesytick',[0.5: plotted_num_events:to_plot_grid*plotted_num_events])
                            set(0,'currentfigure',fh6);
                            
                            flattened = reshape_3Ddata(to_plot, window_around_event,plotted_num_events, to_plot_grid);                            
                            
%                           to_plot = artifactrejection(flattened, window_around_event,-5.5, 5.5, .5, .8, to_plot_grid);
                            imagesc(real(flattened),[-7 7]);
                            if plotted_num_events>1
                                fig_num_y_stacked = [1.1:plotted_num_events:to_plot_grid*plotted_num_events];
                                fig_num_y_stacked = reshape(repmat(fig_num_y_stacked, [to_plot_grid 1]), number_of_electrodes_total, 1);
                                text(fig_num_x2, fig_num_y_stacked, fig_nums)
                                stackedfig_dash_y = repmat([0 plotted_num_events*to_plot_grid],to_plot_grid,1)';
                                line(fig_dash_x2,stackedfig_dash_y,'LineStyle','--','linewidth',1','color',[0 0 0]);
                                
                                
                                y_line=reshape([1:plotted_num_events*16;1:plotted_num_events*16 ],[1,plotted_num_events*16*2]);
                                 y_line=reshape([1.5 :plotted_num_events*16+.5;1.5 :plotted_num_events*16+.5 ],[1,plotted_num_events*16*2]);

                                y_line=[0.5 y_line];
                                if number_of_analog==2 
                                    %keyboard
                                    event2_line_x=window_before_event+eventIndices(3,1:plotted_num_events);
                                    event2_line_x=reshape([event2_line_x; event2_line_x],[1,size(event2_line_x,2)*2])
                                    hold on
                                    
                                    col=1;
                                    for col=1:16
                                        plot(repmat(event2_line_x+(col-1)*window_around_event,[1 16]),y_line(1:end-1),'k','LineWidth',1);
                                    end
                                    

                                    %plot(repmat(event2_line_x,[16,16]).*[0:15].*window_around_event,[1 16]),y_line(1:end-1),'k','LineWidth',1);    
...
%x is essentially plotted_num_events; could probably replace this segment
%{
...
%{
%No longer need this because invalid starting indices will already be
rejected in parse events function
...
%%
function [num_events,indLastEvent,eventIndices,new_event_flag] = parseEvents(ANNewData_finalMAT, ANfinalPos, desired_ANchan,...
...
function [averages, stdevs, medians]=calculate_baseline_angela(time2collectBaseline, ...
    sproj_name,dproj_name, points_needed4baseline, BufferSize, DA, num_channels, num_freq_bands,...
    sampling_rate, number_of_sec2read)
%Instead of doing repmat on baseline samples, I changed it to only keep one
%sample for averages, stdevs. This way, we can do repmat within the main
%function and scale it to the window size we need. This also reduces time
%of saving and loading variable to disk
...
% averages = repmat(mean(logBaselineDataMAT,3),[1 1 (sampling_rate*number_of_sec2read+1)]);
% stdevs = repmat(std(logBaselineDataMAT,0,3),[1 1 (sampling_rate*number_of_sec2read+1)]);
averages = mean(logBaselineDataMAT,3);
stdevs = std(logBaselineDataMAT,0,3);
waitbar(1,h,'Baseline Statistics Gathered!')
pause(1)
close(h)
...
function to_plot = SingleStackedTrials(DataMat, ind, plotted_num_events,...
    averages, stdevs, freq_band, window_around_event)
% function to_plot = SingleStackedTrials(DataMat, ind, ...
%   averages, stdevs, freq_band, window_around_event)
% Creates z-scored matrix (plotted_num_events x time) for ONE channel. Each row
...
to_plot = zeros(size(Data,1),plotted_num_events,window_around_event+1);
empty_trials = [];

%loop through every event and extract data around event
for i=1:plotted_num_events
    beginning = ind(i) - window_around_event/2;
    last = ind(i)+ window_around_event/2;
    if beginning <1 || last>AmountOfData
...
matlab_loopCounter = 0;
eventRelatedAvg=[];
eventRelatedAvg2=eventRelatedAvg;
num_samples=0;
last_used_ind=0;%%
profile on;
good_event_count=0;
