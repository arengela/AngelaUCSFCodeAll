%% get segments for 1 event, 1 Frequency, for selected channels 
%%
allfiles=    {
...
load('E:\DelayWord\EC18\activech')
lh=wordgroups.lh
...
for n=2%[1 2 9]
       ...
       for c=length(activech):-1:1%32:224
                ch=activech(c);
                %ch=c:c+31;
                test.usechans=ch;
               ...
               test.loadBaselineFolder([baseline{n} '/HilbAA_70to150_8band']);
                ...
                for e=2%[2 5]%1:5
                    ...
                    %ps=singleConditionStats(squeeze(test.segmentedEcog(1).zscore_separate(1,:,:,:))',squeeze(test.ecogBaseline.data(1,:,:,:)),1:size(test.segmentedEcog(1).zscore_separate,2))
                    ...
                    load ps
                    psall{e}=ps;
                    data{e}=squeeze(test.segmentedEcog(1).zscore_separate)';
                    
                    %subplot(1,2,e)
                    m= mean(data{e});
                     plot(mean(data{e}),'k')
                    hold on
                    sigidx=find(psall{e}<.0005 & psall{e}~=0);
...
%%Get zscore segments for all LH words


%%Get significant channels for all channels Active 



%%Compare Active and Passive cases (don't need significance for passive
%%case)


%%Get channels with significant difference between Active and Passive


%%Multiple comparison


%%save pvalues as individual channels
