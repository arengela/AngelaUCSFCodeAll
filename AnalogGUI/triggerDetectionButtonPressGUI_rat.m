function handles=triggerDetectionButtonPressGUI_rat(handles)

a=sprintf('Stim%d ',handles.buttonPressChNum);
...
for k=1:length(analogueChannels2Use)
    ...
    if 0
        if checkResultVisually
            ...
            figure;
            r=ceil(-pre*analog_params.sampFreq:(-pre+dur)*analog_params.sampFreq);
