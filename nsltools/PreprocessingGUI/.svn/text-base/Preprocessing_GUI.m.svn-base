...
switch handles.type
    ...
    case 'Rat'
        channelsTot=96;
        timeInt=[];
        handles.ecog=loadHTKtoEcog_rat_CT(sprintf('%s/%s',handles.pathName,'RawHTK'),channelsTot,timeInt);
    case 'DS'
        channelsTot=256;
        timeInt=[];
        handles.ecogDS=loadHTKtoEcog_CT(sprintf('%s/%s',handles.pathName,'ecogDS'),channelsTot,timeInt)
        baselineDurMs=0;
...
if size(handles.ecogDS.data,2)>30000
    tmp=handles.ecogDS;
    tmp.data=handles.ecog.data(:,1:30000);
    checkPeriodogramGUI_figName(tmp,handles.individualPer,'FIG_periodogramDS');

else
    checkPeriodogramGUI_figName(handles.ecogDS,handles.individualPer,'FIG_periodogramDS');
end
