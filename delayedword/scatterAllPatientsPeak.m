    ...
    for i=1:length(patients)-1 
        ...
        for c=1:length(ch)
            ...
            try
                ...
                %plotManyPolygons([(devCS./gridDist)*64;(devSF./gridDist)*64]',i+2,colorjet(idx,:),maxAmp(ch)./5,1.5)
                if s==3000
                    visualizeGrid(2,['E:\General Patient Info\' AllP{i}.Tall{2}.Data.patientID '\brain.jpg'],[]);
                    hold on
                end
                scatter(xyCur(1,:),xyCur(2,:),50,colorjet(idx,:),'fll')
...
colormap(colorjet)
h=colorbar
ylabel(h,'Median Peak Time After Sound Onset(ms)')
